...
function load_save_file() {
	...
	if (file_exists(SAVE_FILE_NAME)) {
		...
		for (var i = 0; i < ds_list_size(_list); i++) {
			...
			with (player) {
				day							= correct_string_after_decode(_map[? "day"]);					
				weekday						= correct_string_after_decode(_map[? "weekday"]);				
				season						= correct_string_after_decode(_map[? "season"]);				
				year						= correct_string_after_decode(_map[? "year"]);					
				name						= _map[? "name"];					
				pronouns					= correct_string_after_decode(_map[? "pronouns"]);
				...
				title						= correct_string_after_decode(_map[? "title"]);		
				firstFloorTileset			= correct_string_after_decode(_map[? "firstFloorTileset"]);
				secondFloorTileset			= correct_string_after_decode(_map[? "secondFloorTileset"]);
				outdoorTileset				= correct_string_after_decode(_map[? "outdoorTileset"]);
				...
				roninScore					= correct_string_after_decode(_map[? "roninScore"]);			
				roninMatchCount				= correct_string_after_decode(_map[? "roninMatchCount"]);		
				roninWinCount				= correct_string_after_decode(_map[? "roninWinCount"]);		
				roninLoseCount				= correct_string_after_decode(_map[? "roninLoseCount"]);		
				onlineRating				= correct_string_after_decode(_map[? "onlineRating"]);			
				onlineMatchCount			= correct_string_after_decode(_map[? "onlineMatchCount"]);		
				onlineWinCount				= correct_string_after_decode(_map[? "onlineWinCount"]);		
				onlineLoseCount				= correct_string_after_decode(_map[? "onlineLoseCount"]);		
